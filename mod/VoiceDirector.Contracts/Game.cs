...
public sealed record DirectorSettings(string Mode = "wildcard", double DrawChance = 0.05, int MaxPerTurn = 1,
...
public sealed record GameSnapshot(string RunId, long Revision, string Phase, int Turn,
    ...
    DirectorSettings Settings, string? Error = null);
...
public interface IGamePort
{
    ...
    Task<CardInstance> Protect(ProtectionRequest request);
}
